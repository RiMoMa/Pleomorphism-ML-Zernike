...
import numpy.matlib
import pandas as pd
from OpenDataPleomorphism import  openDataPleomorphism
...
for clase in range(3):
    ...
    #ImgFiles = ImgFiles[0:10]
    ####
    ListImgNames.extend(ImgFiles)
    Labels = np.matlib.repmat(Clases[clase][-2:-1],1,len(ImgFiles))
...
for train_index, test_index in kf.split(ListImgNames, AllLabels):
    ...
    #Build Dictionary
    #sel randomly n images per class
    y_trainAux = y_train.reset_index().set_index('gradoNP')
    DictionaryIDX = []
