# This script will compute a set of zernike coefficients using a  m decomposition
#and then the image will be reconstructed and psnr mesuare in different stages of pleomorphism
...
for clase in range(1): #sacar las imagenes para el diccionario variando la clase
    ...
    for imn in Nimgs:
        ...
        ZernikePath = FolderOutputZernike + imPath[
                                            len(imPath[0:-12]):-4] + '.pickle'  # file of zernike moments per image
        ...
        if not os.path.exists(DicPath):
            ...
            if not os.path.exists(maskPath):
                ...
                imMask = Image.open(maskPath)  ### Open mask when this exist
            ...
            for imNuclei in regions:  # zernike feature extraction per nucleus
                ...
                NucleusIm = cv2.copyMakeBorder(NucleusIm, 60, 60, 60, 60,
                                               cv2.BORDER_CONSTANT,
                                               value=0)  # build a big patch to avoid small nucleus errors
